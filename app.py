...
if st.button("文字起こしを開始", type="primary"):
    ...
    with st.spinner("AIがPDFを読み取っています...（数分かかる場合があります）"):
        try:
            ...
            # モデルの初期化とテキスト抽出（Gemini 2.5 Flashを適用）
            model = genai.GenerativeModel(model_name="gemini-2.5-flash")
            
            # 構造認識を強制する強力なプロンプト
            prompt = """
            このPDF文書の高精度な文字起こしを行ってください。
            文書は段組み（2段組みなど）になっている場合があります。以下のルールを厳守してテキストを抽出してください：
            1. 【レイアウトの認識】視覚的な段落ブロックと段組みを正確に認識し、人間が自然に読む順序（例：右段を上から下へ読み切ってから、左段を上から下へ）でテキストを構成すること。左右の段のテキストが絶対に混ざらないようにすること。
            2. 【ノイズの排除】ヘッダー（「〔社会保険通報〕」など）やフッター（ページ番号「- 1 -」など）、欄外の不要なテキストは出力から完全に除外すること。
            3. 【構造の維持】見出し、段落の構造、箇条書きを正確に維持すること。
            4. 【表の抽出】表が含まれている場合は、構造を理解し、必ずMarkdown形式の表（テーブル）として綺麗に出力すること。
            5. 【出力形式】余計な挨拶や前置き、説明は一切出力せず、抽出・構成したテキストデータのみを出力すること。
            """
